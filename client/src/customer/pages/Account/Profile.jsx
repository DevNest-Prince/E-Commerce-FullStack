import { Alert, Divider, Snackbar } from "@mui/material";
import { useEffect, useState } from "react";
import { Route, Routes, useLocation, useNavigate } from "react-router-dom";

import Order from "./Order";
import UserDetails from "./UserDetails";
import SavedCards from "./SavedCards";
import OrderDetails from "./OrderDetails";
import Addresses from "./Adresses";

import { useAppDispatch, useAppSelector } from "../../../Redux Toolkit/Store";
import { performLogout } from "../../../Redux Toolkit/Customer/AuthSlice";

const menu = [
  { name: "orders", path: "/account/orders" },
  { name: "profile", path: "/account/profile" },
  { name: "Saved Cards", path: "/account/saved-card" },
  { name: "Addresses", path: "/account/addresses" },
  { name: "Logout", path: "/" },
];

const Profile = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const dispatch = useAppDispatch();

  const { user, orders } = useAppSelector((store) => store);

  const [snackbarOpen, setOpenSnackbar] = useState(false);

  // Logout handler
  const handleLogout = () => {
    dispatch(performLogout());
    navigate("/");
  };

  // Menu click handler
  const handleClick = (item) => {
    if (item.name === "Logout") {
      handleLogout();
      return;
    }
    navigate(item.path);
  };

  // Close Snackbar
  const handleCloseSnackbar = () => {
    setOpenSnackbar(false);
  };

  // Auto trigger Snackbar when updated / canceled / error
  useEffect(() => {
    if (user.profileUpdated || orders.orderCanceled || user.error) {
      setOpenSnackbar(true);
    }
  }, [user.profileUpdated, orders.orderCanceled, user.error]);

  return (
    <div className="px-5 lg:px-52 min-h-screen mt-10">
      {/* User Header */}
      <h1 className="text-xl font-bold pb-5">{user.user?.fullName}</h1>

      <Divider />

      <div className="grid grid-cols-1 lg:grid-cols-3 lg:min-h-[78vh]">
        {/* Left Sidebar Menu */}
        <div className="col-span-1 lg:border-r lg:pr-5 py-5 flex flex-row flex-wrap lg:flex-col gap-3">
          {menu.map((item, index) => {
            const isActive = location.pathname === item.path;

            return (
              <div
                key={index}
                onClick={() => handleClick(item)}
                className={`px-5 py-3 rounded-md cursor-pointer
                  ${isActive ? "bg-primary-color text-white" : "hover:bg-teal-500 hover:text-white"}
                  ${index !== menu.length - 1 ? "border-b" : ""}
                `}
              >
                <p>{item.name}</p>
              </div>
            );
          })}
        </div>

        {/* Right Content Area */}
        <div className="lg:col-span-2 lg:pl-5 py-5">
          <Routes>
            <Route path="/" element={<UserDetails />} />
            <Route path="/orders" element={<Order />} />
            <Route path="/orders/:orderId/item/:orderItemId" element={<OrderDetails />} />
            <Route path="/profile" element={<UserDetails />} />
            <Route path="/saved-card" element={<SavedCards />} />
            <Route path="/addresses" element={<Addresses />} />
          </Routes>
        </div>
      </div>

      {/* Snackbar Alerts */}
      <Snackbar
        anchorOrigin={{ vertical: "top", horizontal: "right" }}
        open={snackbarOpen}
        autoHideDuration={6000}
        onClose={handleCloseSnackbar}
      >
        <Alert
          onClose={handleCloseSnackbar}
          severity={user.error ? "error" : "success"}
          variant="filled"
          sx={{ width: "100%" }}
        >
          {user.error
            ? user.error
            : orders.orderCanceled
            ? "Order canceled successfully"
            : "Success"}
        </Alert>
      </Snackbar>
    </div>
  );
};

export default Profile;
